...
public class GradeEnter extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (e.getSource() == bn) {  //bn是提交按钮
			...
			if (hasThisStu() == 1) {   // 登陆成绩
				...
				try {
					for (int i = 0; i < files.size(); i++) {  // 遍历所有文件
						...
						assert course != null;
						if (course[0].equals(idt.getText())){   //result是课程
							String gradeInfo = idt.getText(); //课程号
							gradeInfo = gradeInfo + " ";
							gradeInfo = gradeInfo + course[1];  //成绩文件里的课程名
							gradeInfo = gradeInfo + " ";
							gradeInfo = gradeInfo + course[2]; //成绩文件里的教师编号
							gradeInfo = gradeInfo + " ";
							gradeInfo = gradeInfo + course[3];  //成绩文件里的教师名
							gradeInfo = gradeInfo + " ";
